...
try:
    ...
    print("Warning: dash_holoniq_wordcloud not installed. Please install it with: pip install dash-holoniq-wordcloud")
    DashWordcloud = None

# Load keywords from file
def load_keywords():
    """Load keywords from sample_keywords.txt"""
    keywords_list = []
...
# Load abstracts to get paper IDs
def load_abstracts():
...
# Generate sample paper data with titles, dates, and citation counts
def generate_paper_data():
    """Generate sample paper data with titles, publish dates, and citation counts"""
    keywords_list = load_keywords()
    abstracts_df = load_abstracts()
    ...
    for idx, keywords in enumerate(keywords_list):
        ...
        abstract = ""
        if not abstracts_df.empty and idx < len(abstracts_df):
            abstract = abstracts_df.iloc[idx]['abstract']
        
        # Generate a title based on keywords or abstract
        if abstract:
            # Extract first sentence or create title from keywords
            first_sentence = abstract.split('.')[0]
            # Clean up and limit length
            title = first_sentence.strip()[:120]
...
app.layout = html.Div([
    ...
    html.Div([
        DashWordcloud(
            ...
            shape='circle',
            hover=True
        ) if DashWordcloud else html.Div("Please install dash_holoniq_wordcloud: pip install dash-holoniq-wordcloud")
    ], style={'display': 'flex', 'justifyContent': 'center', 'marginBottom': '30px'}),
    
    html.Div(id='selected-keyword', 
             style={'textAlign': 'center', 'fontSize': '20px', 'fontWeight': 'bold', 
                    'marginBottom': '20px', 'color': '#3498db'}),
    
    html.Div(id='papers-table-container', style={'margin': '20px auto', 'maxWidth': '1200px'}),
])
...
def update_table(clickData):
    ...
    if selected_keyword not in keyword_to_papers:
        return f"Keyword: {selected_keyword}", html.Div("No papers found for this keyword.")
    ...
    keyword_display = f"Keyword: {selected_keyword.title()} ({len(papers_list)} paper(s))"
    
    return keyword_display, html.Div([
        html.H3(f"Papers containing '{selected_keyword.title()}'", 
